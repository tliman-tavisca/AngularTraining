...
@customElement("ifdefined-element")
class IfDefinedElement extends LitElement {
  @state()
  ...
  render() {
    ...
    return html`
      ...
      <input id="name" .value=${this.imageName} @input=${this.updateImage} />
      <hr />
      <img
        src="https://${ifDefined(info?.domain)}/id/${ifDefined(
          info?.id
        )}/200/200"
      />
      <hr />
